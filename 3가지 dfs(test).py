...
# (선택) 스크립트 폴더를 작업 디렉터리로 고정
try:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    os.chdir(script_dir)
except NameError:
    pass  # 인터프리터에서 직접 실행 시 __file__이 없을 수 있음

# 1) 그래프 로딩 + 중복 제거 + 이웃 정렬
def load_graph(path='testgraph_medium.txt'):
    raw = defaultdict(set)
...
if __name__ == "__main__":
    graph = load_graph('testgraph_medium.txt')
    # 세 방식 모두 컴포넌트별 방문 순서 출력
